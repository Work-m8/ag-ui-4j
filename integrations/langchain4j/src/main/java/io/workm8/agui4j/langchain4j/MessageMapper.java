...
public class MessageMapper {
    ...
    /**
     * Converts an agui4j BaseMessage to the corresponding LangChain4j ChatMessage.
     * <p>
     ...
     * </ul>
     *
     * @param message the agui4j BaseMessage to convert
    ...
    public dev.langchain4j.data.message.ChatMessage toLangchainMessage(final io.workm8.agui4j.core.message.BaseMessage message) {
        switch (message.getRole()) {
            ...
            default -> {
                return this.toLangchainUserMessage((io.workm8.agui4j.core.message.UserMessage) message);
            }
        }
    }

    /**
     * Converts an agui4j ToolMessage to a LangChain4j ToolExecutionResultMessage.
     * <p>
     ...
     *
     * @param message the agui4j ToolMessage containing tool execution result
     * @return a LangChain4j ToolExecutionResultMessage with the tool call ID,
     *         name, and execution result content
     */
    public dev.langchain4j.data.message.ToolExecutionResultMessage toLangchainToolMessage(final io.workm8.agui4j.core.message.ToolMessage message) {
    ...
    /**
     * Converts an agui4j UserMessage to a LangChain4j UserMessage.
     * <p>
     ...
     */
    public dev.langchain4j.data.message.UserMessage toLangchainUserMessage(final io.workm8.agui4j.core.message.UserMessage message) {
    ...
    /**
     * Converts an agui4j SystemMessage to a LangChain4j SystemMessage.
     * <p>
     ...
     */
    public dev.langchain4j.data.message.SystemMessage toLangchainSystemMessage(final io.workm8.agui4j.core.message.SystemMessage message) {
        return dev.langchain4j.data.message.SystemMessage.systemMessage(message.getContent());
    }

    /**
     * Converts an agui4j AssistantMessage to a LangChain4j AiMessage.
     * <p>
     ...
     * <li>Converts agui4j tool calls to LangChain4j ToolExecutionRequests</li>
     ...
     * </ul>
     * <p>
     * Tool calls are converted from agui4j's ToolCall format to LangChain4j's
     * ToolExecutionRequest format, ensuring compatibility with LangChain4j's
     * function calling mechanisms.
     *
     * @param message the agui4j AssistantMessage to convert, potentially containing
     ...
     */
    public dev.langchain4j.data.message.AiMessage toLangchainAiMessage(final io.workm8.agui4j.core.message.AssistantMessage message) {
